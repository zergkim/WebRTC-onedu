...
const Get_jungbo = async(filename:string)=>{
    const ObjID:mongodb.ObjectID= new ObjectID(filename)
    await DBobj.Videodata.findOne({_id:ObjID})
}
const FindUser = async(username:string) => await DBobj.Users.findOne({username});
const splite = (name:string,d:string)=>{
...
function postthedata(vdata:Buffer,idata:Buffer,post_data_obj:POST_DATA_OBJ){
